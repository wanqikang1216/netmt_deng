...
extern struct thread_master *g_cmdmaster;
...
vty_event (enum event event, int sock, struct vty *vty)
{
  ...
  switch (event)
    {
    case VTY_SERV:
      vty_serv_thread = thread_add_read (g_cmdmaster, vty_accept, vty, sock);
      vector_set_index (Vvty_serv_thread, sock, vty_serv_thread);
      break;
#ifdef VTYSH
    case VTYSH_SERV:
      thread_add_read (g_cmdmaster, vtysh_accept, vty, sock);
      break;
    case VTYSH_READ:
      thread_add_read (g_cmdmaster, vtysh_read, vty, sock);
      break;
#endif /* VTYSH */
    case VTY_READ:
      vty->t_read = thread_add_read (g_cmdmaster, vty_read, vty, sock);
      ...
      if (vty->v_timeout)
	{
	  if (vty->t_timeout)
	    thread_cancel (vty->t_timeout);
	  vty->t_timeout = 
	    thread_add_timer (g_cmdmaster, vty_timeout, vty, vty->v_timeout);
	}
      break;
    case VTY_WRITE:
      if (! vty->t_write)
	vty->t_write = thread_add_write (g_cmdmaster, vty_flush, vty, sock);
      break;
